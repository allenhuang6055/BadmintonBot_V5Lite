const { getEnabledItems, appendRecords, getSummary, getCurrentStock } = require("../services/googleSheet");
...
async function handleIncome(text, user) {
  ...
  for (const item of items) {
    ...
    if (amount > 0) records.push({ type: "收入", item, income: amount, note });
  }
  if (ballsUsed > 0) records.push({ type: "庫存", item: "耗球", ballsUsed, note });
  if (!records.length) throw new Error("沒有讀到收入金額或耗球數。請確認格式，例如：零打：500、耗球：18");
...
module.exports = { parseAmount, parseNote, incomeTemplate, isIncomeRecord, handleIncome };
